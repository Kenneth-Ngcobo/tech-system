...
<table v-if="!wait" class="table table-bordered table-hover" style="font-size:12px; width:100%; margin-top:1%;">
        ...
        <tr>
            ...
            <th>Quote  <span class="dropdown" style="float:right" > 
                ...
                <div :class="{'dropdown-menu' : true, 'show': show}" style="width:350px">
                    ...
                    <input type="checkbox" :value="q" v-model="selected_quotes" @change="filterQuotes3($event)">
    ...
    <tbody>
        ...
        <tr  :key="q.ItemID" :class="{ red: checkCodes(q.Code)}">
            ...
            <td style="width:4%; text-align:center">{{index+1}} <input type="checkbox" v-model="q.selected"></td>
            <td style="width:10%"><strong>{{q.QuotationID.toString().padStart(8, '0')}}</strong> ({{q.Quote}} - V{{q.Version}})</td>
            ...
            <td style="width:4%; text-align:center" v-else><select style="width:100%" v-model="q.Warehouse" @change=" ItemWarehouse(q)">
                <option value="">Select</option>
                <option v-for="wh in warehouse" :key="wh.Code" :value="wh.Code">{{wh.Code}} - {{wh.Name}}</option>
            </select></td>
            ...
            <td style="width:4%; text-align:center">R{{q.UnitCost}}</td>
         ...
         <tr v-for="(acc,index2) in q.accessories" :key="acc.ItemID" :class="{ red: checkCodes(acc.Code)}">
            <td></td>
            <td></td>
            <td>{{acc.Code}}</td>
            <!--td v-else><input type="text" v-model="acc.Code"></td-->
            <td>{{acc.Description}}</td>
            <td style="text-align:center">{{acc.Warehouse}}</td> <!--v-if="!edit && !edit_qty"-->
            <!--td style="width:3%; text-align:center" v-else><select style="width:100%" v-model="acc.Warehouse">
                <option value="">Select</option>
                <option v-for="wh in warehouse" :key="wh.Code" :value="wh.Code">{{wh.Code}} - {{wh.Name}}</option>
            </select></td-->
            <td v-if="!edit" style="text-align:center">{{acc.Quantity}}</td>
            <td style="width:3%; text-align:center" id="mod" v-else><input type="text" style="width:100%" v-model="acc.Quantity"></td>
            <td></td>
            <td></td>
            <td></td>
        </tr>
            ...
            <table>
             ...
             <!--table>
                <tr>
                    <td><label>Send Email To: </label>
                        <select  v-model="email">
                            <option v-for="email in admin_emails" :key="email.id" :value="email.email">{{email.email}}</option>
    ...
    methods: {
        ...
        async filterQuotes3() {
            ...
            for(let i = 0; i<this.selected_quotes.length; i++) {
                   ...
                   if(count == 0) {
                    ...
                    quotes = this.quotes.filter(q => q.QuotationID == this.selected_quotes[i].QuotationID)
                    if(quotes.length == 0) {
                        let res = await this.$axios.$post('/project_quotes', {quotes: this.selected_quotes[i].QuotationID})
                        if(res.hasOwnProperty('type')) {
                            ...
                            //this.quotes = this.quotes.concat(res)
                            quotes = res
                        }
                    }
                    ...
                    quote2 = this.quotes.filter(q => q.QuotationID == this.selected_quotes[i].QuotationID)
                    ...
                    if(quote2.length == 0) {
                        ...
                        if(res.hasOwnProperty('type')) {
                            ...
                            this.quotes = this.quotes.concat(res)
                            quotes = quotes.concat(res)
                        }
                    }
                    else {
                        quotes = quotes.concat(quote2)
                    }
       ...
       Position(name) {
        ...
        async processSO() {
            ...
            if(warehouse) {
                ...
                for(let i = 0; i<list.length; i++) {
                    ...
                    dump[i].ResponsibleEmail = this.$auth.user.email
                    dump[i].ExternalOrder = this.so.ext_ordernum
                    dump[i].OrderDate = this.today() 
                    dump[i].StockCode = list[i].Code
                    dump[i].Description = list[i].Description.split("\n")[0].trim()
                    //console.log(dump[i].Description)
                    dump[i].WarehouseCode = list[i].Warehouse
                    dump[i].Quantity = list[i].Quantity
                    dump[i].PriceExcl = list[i].UnitCost*((100 - (list[i].DiscountPercentage+this.quotes[0].Discount))/100)
                    dump[i].PriceInc = dump[i].PriceExcl*1.15
                ...
                /*if(this.loaded == "project") {
                    ...
                    await this.$axios.$post('/update_sfa', {type: "project", no: this.project, status: 5})
                }
                if(this.loaded == "quote") {
                
                    await this.$axios.$post('/update_sfa', {type: "quote", no: this.quote_no, status: 12})
                }*/
               ...
               /* var sheet = XLSX.utils.json_to_sheet(dump);
                for(let i in sheet) {
                    if(i != '!ref') {
                        //console.log(i)
                        sheet[i].s = {
                ...
                XLSX.utils.book_append_sheet(wb, sheet, `${this.sfa_ref}`);
                XLSX.writeFile(
                    wb,
                    `${this.sfa_ref}.csv`
                ); */
        ...
        async loadQuotes() {
            ...
            this.codes = await this.$axios.$post('/codes', obj)
            ...
            if(this.quote_versions.length != 1) {
                ...
                get_quotes = this.quote_versions.filter(q => q.IsLatest == true && q.UseToEstimate == true)
                if(get_quotes.length == 0) {
                    get_quotes = this.quote_versions.filter(q => q.IsLatest == true)
                }
                if(get_quotes.length == 0) {
                    get_quotes = this.quote_versions
                }
            }
            else {
                    get_quotes = this.quote_versions
            }
            this.selected_quotes.push(get_quotes[0])
            this.selected_array = JSON.parse(JSON.stringify(get_quotes))

            this.quote_ids = get_quotes.map(s => s.QuotationID)
            //let quotes = this.selected_quotes.map(s => s.QuotationID)
            //let res = await this.$axios.$post('/project_quotes', {quotes: JSON.stringify(quotes)})
            let res = await this.$axios.$post('/project_quotes', {quotes: this.quote_ids[0]})
            if(res.hasOwnProperty('type')) {
                alert(`${res.message}`)
                this.wait = false
                return
            ...
            this.quotes = res//await this.$axios.$post('/project_quotes', {quotes: JSON.stringify(quotes)}) / Quotes
            }
            ...
            this.quotes.forEach(q => {
                ...
                if(myquote != undefined) { //Code exists on mysql
                    code_exists = true
                ...
                if(q.AConcatCode != null) {
                    if(q.AConcatCode != "") {
                    let acc = q.AConcatCode.split("|")
                    let length = acc.length
                    if(acc[acc.length-1].length == 0) {
                        length = length-1
                    }
                    for(let i = 0; i<length; i++) {
                        let acc_code = acc[i].split(">")[0]
                        let acc_qty = acc[i].split(">")[2]
                        let description = this.accDescription(acc_code)
                        //console.log(String.fromCharCode(i + 64))
                        let a = String.fromCharCode(i+1 + 64)
                        /*a.toLowerCase()
                        console.log(a)*/
                        //this.accessories.push({id: q.ItemID, code: acc_code, qty: acc_qty})
                        q.accessories.push({ItemID: q.ItemID+a, Warehouse: q.Warehouse, QuotationID: q.QuotationID, Quote: q.Quote, Version: q.Version, Code: acc_code, Description: description, Quantity: acc_qty*q.Quantity, UnitCost:0})
